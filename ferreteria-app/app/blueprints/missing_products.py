"""Missing Products blueprint for tracking customer requests for unavailable products."""
...
def list_missing_products():
    """List missing product requests with filters and search."""
    ...
    try:
        ...
        # Base query
        query = db_session.query(MissingProductRequest)
...
def register_request():
    """Register a new missing product request or increment existing one."""
    db_session = get_session()
    ...
    try:
        ...
        # Check if product already exists in catalog (optional warning)
        ...
        if existing_product:
            flash(
                f'⚠️ Advertencia: El producto "{existing_product.name}" ya existe en el catálogo (ID: {existing_product.id}). '
                f'Considera usar el producto existente en vez de registrarlo como faltante.',
                'warning'
            )
            # Allow registering anyway but show warning
        
        # Check if already exists in missing products
        existing_req = db_session.query(MissingProductRequest).filter_by(
...
def resolve_request(request_id):
    """Mark a missing product request as RESOLVED."""
    db_session = get_session()
    
    try:
        missing_req = db_session.query(MissingProductRequest).filter_by(id=request_id).first()
...
def reopen_request(request_id):
    """Reopen a resolved missing product request."""
    db_session = get_session()
...
def update_notes(request_id):
    """Update notes for a missing product request."""
    db_session = get_session()
