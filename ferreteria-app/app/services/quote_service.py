...
def generate_quote_number(session) -> str:
    """
    Generate a unique quote number with format: PRES-YYYYMMDD-HHMMSS-####
...
def create_quote_from_cart(cart: dict, session, customer_name: str, customer_phone: str = None, payment_method: str = None, notes: str = None, valid_days: int = 7) -> int:
    """
    ...
    Args:
        ...
        session: SQLAlchemy session
        customer_name: Customer name (required) - MEJORA 14
        customer_phone: Customer phone (optional) - MEJORA 14
    ...
    try:
        ...
        # MEJORA 14: Validate customer_name
        ...
        # Create quote lines with snapshot
        total = Decimal('0.00')
...
def generate_quote_pdf_from_db(quote_id: int, session, business_info: dict) -> BytesIO:
    """
    Generate PDF from a persisted quote in database.
...
def convert_quote_to_sale(quote_id: int, session) -> int:
    """
    Convert a quote to a sale with full transactional processing.
    
    Steps:
    1. Lock quote row (FOR UPDATE)
    2. Validate quote status and conditions
    3. For each line:
       - Lock product_stock (FOR UPDATE)
       - Validate stock availability
    4. Create sale + sale_lines
    5. Create stock_move OUT + stock_move_lines
    6. Create finance_ledger INCOME
    ...
    try:
        ...
        # Step 3: Validate stock availability for all lines
        for line in quote.lines:
            # Lock product_stock for update
            product_stock = (
                session.query(ProductStock)
                .filter(ProductStock.product_id == line.product_id)
                .with_for_update()
                .first()
            )
        ...
        # Step 5: Create sale_lines from quote_lines
        for line in quote.lines:
            sale_line = SaleLine(
                sale_id=sale.id,
        ...
        # Step 7: Create stock_move_lines
        for line in quote.lines:
            product = session.query(Product).filter_by(id=line.product_id).first()
